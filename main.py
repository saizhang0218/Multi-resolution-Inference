...
parser.add_argument('-b', '--batch-size', default=160, type=int,metavar='N', help='mini-batch size (default: 256)')
...
parser.add_argument('-lt', '--loss_type', default='joint', help='two options: joint, distillation')
...
'''
The list of weight term budget and data term budget.
For example, [[a,b],[c,d]] represents two sub-models.
The first sub-model has a data term budget of a and weight term budget of b
The second sub-model has a data term budget of c and weight term budget of d
'''
budget_list = [[2,8],[2,10],[2,12],[2,14],[3,14],[3,16],[3,18],[3,20]]
...
def main_worker(gpu, ngpus_per_node, args, fdir):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        top1_m_avg_list = validate(val_loader, model, criterion, args)
        
        for j in range(len(top1_m_avg_list)):
            f = open('./accuracy/' + args.save_path + '/' + 'data-budget-' + str(budget_list[j][0])  + 'wgt-budget-' + str(budget_list[j][1]) + '_acc.txt', "a")
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer):
    ...
    budget_list_temp = budget_list[0:-1]
    counter = 0
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        if (counter == len(budget_list_temp)):
            random.shuffle(budget_list_temp)
            counter = 0
        
        # pick the budgets of the teacher model and student model
        budget_teacher = budget_list[-1]
        budget_student = budget_list_temp[counter]
        counter = counter + 1    
        
        output1 = model(images, budget_teacher[0], budget_teacher[1])   # 0 means the floating-point model
        output2 = model(images, budget_student[0], budget_student[1])
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            for j in range(len(budget_list)):
                # compute output
                output = model(images, budget_list[j][0], budget_list[j][1])
        ...
        for j in range(len(budget_list)):
            ...
            print(' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f} Act Budget {act_budget:.1f} Wgt Budget {wgt_budget:.1f}'.format(top1=top1_m_list[j], top5=top5_m_list[j], act_budget=budget_list[j][0], wgt_budget=budget_list[j][1]))
    model.module.show_params()
